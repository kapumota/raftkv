...
El runner funciona en Linux con Go y Docker local. Usa los contenedores
...
Esta guía describe ejecuciones manuales sobre un despliegue existente. La
campaña reproducible final se ejecuta con `make experiment`; ese flujo usa un
proyecto Compose dedicado y volúmenes nuevos para cada run.
...
/tmp/raftkv-experiment \
  -scenario experiments/configs/leader-partition-5.yaml \
  -check
```
...
`127.0.0.1`. Compose puede recrear contenedores al añadir los puertos; en este
modo manual conserva los volúmenes WAL existentes.
...
La evidencia dirigida de correctness se documenta en `CORRECTNESS_INVARIANTS.md`
y `EVIDENCE_MATRIX.md`. El runner tampoco verifica automáticamente el estado final
de cada clave.
